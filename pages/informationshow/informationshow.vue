<template>
	<view>
		<view>
			<view class="headimage">
				<image class="image" src="https://aaa-1302259509.cos.ap-beijing.myqcloud.com/%E5%BE%AE%E4%BF%A1%E5%9B%BE%E7%89%87_20200612171625.png"
				 mode="widthFix"></image>
			...
			<view class="user">
				<view class="part1">
					<text style="font-size:50rpx;font-weight:550;">{{yourData.yourName}}</text>
					<text>{{yourData.yourDepartment}}<text>{{yourData.yourClass}}</text></text>
				</view>
				<view>
					<van-cell is-link title="修改个人信息" @click="toInformation" />
					<van-cell is-link title="关于我们" @click="toAboutUs" />
				</view>
			</view>
		</view>
	...
	<!-- 	<view>
		<view>
			<view>
				<p>学校：{{yourData.yourSchool}}</p>
				<p>院系：{{yourData.yourDepartment}}</p>
				<p>班级：{{yourData.yourClass}}</p>
				<p>学号：{{yourData.yourID}}</p>
				<p>姓名：{{yourData.yourName}}</p>
			</view>
			<view>
				<button @click="toInformation">修改个人信息</button>
			</view>
		</view>
		<p hidden>{{randomNum}}</p>
	</view> -->
...
<script>
	export default {
		...
		onShow: function() {
			let _this = this;
			console.log('informationshow show');
			_this.yourData = {
				_id: getApp().globalData.yourData._id,
				wechatId: getApp().globalData.yourData.wechatId,
				yourSchool: getApp().globalData.yourData.studentSchool,
